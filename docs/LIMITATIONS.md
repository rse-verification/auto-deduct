...
AutoDeduct V1 is a deterministic CLI pipeline. It does not modify source
...
V1 accepts one C translation unit plus its headers. Multiple `.c` inputs are
...
| Nested pointers | May fail during functional inference, ISP, contract checking, or WP. V1 does not infer a complete multi-level validity and aliasing model. | Flatten the interface where appropriate or write explicit contracts for every dereference level. |
...
| Loops | General loop-invariant inference is outside V1. Missing or insufficient annotations leave WP goals unresolved. | Add reviewed `loop invariant`, `loop assigns`, and, where required, `loop variant` clauses. |
